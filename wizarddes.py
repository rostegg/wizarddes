...
class TokensType:
    ...
    TOKENS_WITH_VALUES = [ID, REGEX, CONTAINS, FULL]
...
data_proc operators:
ALL|SINGLE BY ID|REGEX|CONTAINS|FULL (value:str) -> CLOSE|MV_SEPARATE(...,desktopRange)|MV_TO(...,desktopID) (ID) 
'''
...
class TokenParser:
    ...
    def is_token_with_value(self, token):
        # old regex which don't parse regex spec symbols as value
        #reg = r"(?P<token>[\S]+)\(\W?(?P<tokenValue>[\w\s/\\,.-]+)\W?\)"
